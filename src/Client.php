...
use Noilty\Http\Client\Exception\Error;
use Noilty\Http\Client\Exception\ResponseError;
use CurlHandle;
...
class Client
{
    ...
    /**
     * Set a username and password (Basic authentication).
     * Not recommended way to authenticate.
     */
    ...
    ): Response
    {
        ...
        if (
            ...
            $string = $method . ' /' . $path;
            $hashHmac = hash_hmac('sha256', $string, $this->secretKey, true);
            $authPart = base64_encode($this->apiKey . ':' . $hashHmac);
        ...
        $errorMessage = $responseHeaders['X-Status-Reason'] // TODO
            ?? $response->getParsedBody()->message
            ?? $response->getParsedBody()->error
            ?? '';

        throw new ResponseError($response, $errorMessage, $responseCode);
    }
